...
define('ga_email','tvimp3.sm@gmail.com');
define('ga_password','tvi$mail1');
define("MAX_RESULTS", 30);
require 'gapi.class.php';
$ids = array();
$result2 = mysql_query("SELECT `Google Profile ID` FROM `dealer-tvimp3.sm` WHERE `Timestamp` < now() - INTERVAL 2 DAY ORDER BY `Index` ASC LIMIT ".MAX_RESULTS);
...
{
echo $id."<br/>";
$SQL = "UPDATE `dealer-tvimp3.sm` SET `Report done` = `Report done` + 1 WHERE `Google Profile ID` = '$id'";			
		if (mysql_query($SQL)) {
			echo "saved";
		} else {
...
$ga->requestReportData($id,array('day', 'month','year'),array('pageviews','visits','bounces','visitors','avgTimeOnPage','pageviewsPerVisit', 'percentNewVisits', 'visitBounceRate'),array('-year','-month') ,$filter=null,$start_date=date('Y-m-d',strtotime('2 days ago')),$end_date=$today,$start_index=1,$max_results=10);
...
$SQL = "INSERT INTO `visits` (`Google Profile ID`, `Date`, `Visits`, `Pageviews`, `Bounces`, `Unique Visitors`, `Avg Time on Page (seconds)`, `Page View Per Visit`, `Bounce Rate`, `% New Visits`, `type of report`)";
		$SQL .= " VALUES ";
		$SQL .= "('$id', '$date', '$visit', '$pageview', '$bounce', '$visitor', '$timeonpage', '$viewpervisit', '$bouncerate', '$percentnewvisit', '2 day report')";
			// Insert the values
		if (mysql_query($SQL)) {
			echo "saved";
		} else {
...
/* *********************************query mobile trafic******************************************************************************************/ 
$ga->requestReportData($id,array('mobileDeviceInfo', 'source'),array('visits','pageviews', 'timeOnSite'),array('-visits') ,$filter=null,$start_date=date('Y-m-d',strtotime('2 days ago')),$end_date=$today,$start_index=1,$max_results=100);
...
$SQL = "INSERT INTO `mobile device` (`Google Profile ID`, `devices`, `source`,`visits`,`pageviews`,`time on site`, `type of report`)";
		$SQL .= " VALUES ";
		$SQL .= "('$id', '$mobileDeviceInfo','$source','$visits','$pageviews','$timeOnSite', '2 day report'                               )";
			// Insert the values
		if (mysql_query($SQL)) {
			echo "saved";
		} else {
...
$ga->requestReportData($id,array('browser','operatingSystem'),array('visits', 'percentNewVisits', 'newVisits'),array('-visits') ,$filter=null,$start_date=date('Y-m-d',strtotime('2 days ago')),$end_date=$today,$start_index=1,$max_results=100);
...
$SQL = "INSERT INTO `topbrowsers` (`Google Profile ID`,`name`,`os`,`visits`,`new visit`,`% new visits`, `type of report`)";
		$SQL .= " VALUES ";
		$SQL .= "('$id', '$browser','$operatingSystem','$visit','$newvisit','$percentnewvisit', '2 day report')";
			// Insert the values
		if (mysql_query($SQL)) {
			echo "saved";
		} else {
...
<?php
/* *********************************Referring Sites******************************************************************************************/ 
$ga->requestReportData($id,array('source'),array('visits', 'timeOnSite', 'exits'),array('-visits') ,$filter='medium==referral',$start_date=date('Y-m-d',strtotime('2 days ago')),$end_date=$today,$start_index=1,$max_results=100);
...
$SQL = "INSERT INTO `referring sites` (`Google Profile ID`,`referring source`,`visits`, `time on site`, `exits`, `type of report`)";
		$SQL .= " VALUES ";
		$SQL .= "('$id', '$source', '$visit', '$timeOnSite','$exits', '2 day report')";
			// Insert the values
		if (mysql_query($SQL)) {
			echo "saved";
		} else {
...
<?php
/* *********************************Organic Search******************************************************************************************/ 
$ga->requestReportData($id,array('source'),array('visits', 'timeOnSite', 'exits'),array('-visits') ,$filter='medium==organic',$start_date=date('Y-m-d',strtotime('2 days ago')),$end_date=$today,$start_index=1,$max_results=100);
...
<?php
/* *********************************Paid Search******************************************************************************************/ 
$ga->requestReportData($id,array('source'),array('visits', 'timeOnSite', 'exits'),array('-visits') ,$filter='medium==cpa,medium==cpc,medium==cpm,medium==cpp,medium==cpv,medium==ppc',$start_date=date('Y-m-d',strtotime('2 days ago')),$end_date=$today,$start_index=1,$max_results=100);
...
<?php
/* *********************************Paid keywords******************************************************************************************/ 
$ga->requestReportData($id,array('adKeywordMatchType'),array('visits', 'percentNewVisits', 'newVisits'),array('-adKeywordMatchType') ,$filter=null,$start_date=date('Y-m-d',strtotime('2 days ago')),$end_date=$today,$start_index=1,$max_results=100);
...
$SQL = "INSERT INTO `ad words` (`Google Profile ID`,`keywords`,`visits`,`new visits`,`% new visits`, `type of report`)";
		$SQL .= " VALUES ";
		$SQL .= "('$id', '$adKeywordMatchType', '$visit', '$newvisit','$percentnewvisit', '2 day report')";
			// Insert the values
		if (mysql_query($SQL)) {
			echo "saved";
		} else {
...
<?php
/* *********************************Organic Keywords******************************************************************************************/
$ga->requestReportData($id,array('keyword'),array('visits', 'percentNewVisits', 'newVisits'),array('-visits') ,$filter='medium==organic',$start_date=date('Y-m-d',strtotime('2 days ago')),$end_date=$today,$start_index=1,$max_results=100);
...
$SQL = "INSERT INTO `organic keywords` (`Google Profile ID`,`keywords`,`visits`,`new visits`,`% new visits`, `type of report`)";
		$SQL .= " VALUES ";
		$SQL .= "('$id', '$keyword', '$visit', '$newvisit','$percentnewvisit', '2 day report')";
			// Insert the values
		if (mysql_query($SQL)) {
			echo "saved";
		} else {
...
<?php
/* *********************************Direct Landing Page******************************************************************************************/ 
$ga->requestReportData($id,array('landingPagePath'),array('visits', 'pageviews', 'newVisits','uniquePageviews', 'avgTimeOnPage'),array('-visits') ,$filter=null,$start_date=date('Y-m-d',strtotime('2 days ago')),$end_date=$today,$start_index=1,$max_results=10);
...
$SQL = "INSERT INTO `top landing pages` (`Google Profile ID`,`landing page path`,`visits`,`new visits`,`pageviews`,`unique pageviews`, `avg time on page (s)`, `type of report`)";
		$SQL .= " VALUES ";
		$SQL .= "('$id', '$landingPagePath', '$visit', '$newvisit','$pageviews', '$uniquePageviews','$avgTimeOnPage', '2 day report')";
			// Insert the values
		if (mysql_query($SQL)) {
			echo "saved";
		} else {
...
<?php
/* *********************************Event Tracking******************************************************************************************/ 
$ga->requestReportData($id,array('eventLabel'),array('totalEvents', 'uniqueEvents', 'visitsWithEvent', 'avgEventValue'),array('-eventLabel') ,$filter=null,$start_date=date('Y-m-d',strtotime('2 days ago')),$end_date=$today,$start_index=1,$max_results=100);
...
$SQL = "INSERT INTO `event tracking` (`Google Profile ID`,`event label`,`total events`,`unique events`,`visits with events`,`avg event value`, `type of report`)";
		$SQL .= " VALUES ";
		$SQL .= "('$id', '$eventLabel', '$totalEvents', '$uniqueEvents','$visitsWithEvent', '$avgEventValue', '2 day report')";
			// Insert the values
		if (mysql_query($SQL)) {
			echo "saved";
		} else {
