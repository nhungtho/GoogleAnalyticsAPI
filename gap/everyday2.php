...
$ids = array();
$result2 = mysql_query("SELECT `Google Profile ID` FROM `dealer-tvimp3.sm` WHERE `Timestamp` < now() - INTERVAL 1 DAY ORDER BY `Index` ASC LIMIT ".MAX_RESULTS);
...
$ga->requestReportData($id,array('day', 'month','year'),array('pageviews','visits','bounces','visitors','avgTimeOnPage','pageviewsPerVisit', 'percentNewVisits', 'visitBounceRate'),array('-year','-month') ,$filter=null,$start_date=date('Y-m-d',strtotime('1 day ago')),$end_date=$today,$start_index=1,$max_results=10);
